...
local SPELL_TYPES = {
	...
	["Curse of the Elements"] = "CURSE",
	["Curse of Shadow"] = "CURSE",
	["Curse of Exhaustion"] = "CURSE",
	-- CC
	["Fear"] = "CC",
	["Howl of Terror"] = "CC",
	["Death Coil"] = "CC",
...
-- Funcion para obtener el color segun el tipo de hechizo
function DoTimer_GetSpellColor(spellname)
	local spelltype = SPELL_TYPES[spellname]
	if spelltype == "DOT" then
		return COLOR_DOT
	elseif spelltype == "CURSE" then
		return COLOR_CURSE
	elseif spelltype == "CC" then
		return COLOR_CC
	elseif spelltype == "PET" then
		return COLOR_PET
	elseif spelltype == "DRAIN" then
		return COLOR_DRAIN
	else
		return COLOR_DEBUFF
	end
...
-- Sonidos por tipo de hechizo (El Sequito del Terror)
-- Usa sonidos del juego para diferentes tipos de alertas
local SPELL_SOUNDS = {
	["DOT"] = "Sound\\Spells\\ShadowWordPain.wav",
	["CURSE"] = "Sound\\Spells\\Curse.wav",
	["CC"] = "Sound\\Spells\\Fear.wav",
	["PET"] = "Sound\\Spells\\SpellLock.wav",
	["DRAIN"] = "Sound\\Spells\\DrainLife.wav",
	["DEFAULT"] = "Interface\\Addons\\DoTimer\\Extras\\expalert.wav",
}

-- Funcion para reproducir sonido segun el tipo de hechizo
function DoTimer_PlaySpellSound(spellname)
	if not DoTimer_Settings.playsound then return end
	local spelltype = SPELL_TYPES[DoTimer_ReturnEnglish(spellname)]
	local soundfile = SPELL_SOUNDS[spelltype] or SPELL_SOUNDS["DEFAULT"]
				...
				for id = 1,table.getn(casted[i]) do --for each debuff on the target
					if DoTimer_Settings.format == "icons" then
						...
						elseif remaining <= 5 and DoTimer_Settings.expalert then r,g,b = COLOR_RED.r,COLOR_RED.g,COLOR_RED.b
							...
							local spellcolor = DoTimer_GetSpellColor(DoTimer_ReturnEnglish(casted[i][id].spell))
							r,g,b = spellcolor.r, spellcolor.g, spellcolor.b	
						end
